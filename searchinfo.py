...
def load_config():
    ...
    try:
        ...
        # 确保必要的目录存在
        for dir_name in ['log_dir', 'data_dir', 'output_dir']:
            dir_path = CONFIG.get('Paths', dir_name, fallback=dir_name)
            os.makedirs(dir_path, exist_ok=True)
        
        # 初始化日志
        log_path = os.path.join(
            CONFIG.get('Paths', 'log_dir'),
            f"cert_check_{datetime.now().strftime('%Y%m%d')}.log"
        )
...
# 代理配置
PROXY_CONFIG = {
    "enable": CONFIG.getboolean('Proxy', 'enable', fallback=False),
    "type": CONFIG.get('Proxy', 'type', fallback='http'),
    "server": CONFIG.get('Proxy', 'server', fallback='127.0.0.1'),
